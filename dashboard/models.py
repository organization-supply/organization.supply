...
class Mutation(TimeStampedModel):
    ...
    contra_mutation = models.ForeignKey("self", on_delete=models.CASCADE, blank=True, null=True)
...
class Inventory(models.Model):
    ...
    def add(self, amount: float) -> Mutation:
        ...
        mutation = Mutation(operation='add', location=self.location, product=self.product, amount=amount)
        mutation.save()
    ...
    def remove(self, amount: float) -> Mutation:
        ...
        mutation = Mutation(operation='remove', location=self.location, product=self.product, amount=amount)
        mutation.save()
        self.amount = self.amount - amount
        self.save()
    ...
    def transfer(self, amount: float, other_location: Location) -> (Mutation, Mutation):
        ...
        other_inventory, created = Inventory.objects.get_or_create(product=self.product, location=other_location)
